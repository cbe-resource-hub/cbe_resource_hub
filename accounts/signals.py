...
def auto_set_admin_role(
    ...
    if instance.is_superuser and instance.role != CustomUser.Role.ADMIN:
        ...
        sender.objects.filter(pk=instance.pk).update(role=CustomUser.Role.ADMIN)
        logger.info("auto_set_admin_role: set role=admin for %s", instance.email)
...
def ensure_superuser_email_verified(
    ...
    try:
        ...
        logger.exception(
            "ensure_superuser_email_verified: failed for %s", instance.email
        )
